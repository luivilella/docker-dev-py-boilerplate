...
DB_ENV_TEMPLATE        = "$(APP_LOCATION)/docker/dev/db/.env.template"
DB_ENV_DEST            = "$(APP_LOCATION)/docker/dev/db/.env"

APP_ENV_TEMPLATE       = "$(APP_LOCATION)/docker/dev/application/.env.template"
APP_ENV_DEST           = "$(APP_LOCATION)/docker/dev/application/.env"

RUNSERVER              = $(APPLICATION_RUNSERVER)
...
up: start ## Start containers and run the project in dev mode
	@docker-compose exec app $(RUNSERVER)
...
run: ## Run a command inside app container: make run-app cmd="python ..."
